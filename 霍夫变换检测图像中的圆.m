实现了基于霍夫变换的findcircle函数，function[circlefind]=findcircle(img,minr,maxr,stepr,stepa,percent)传进来6个参数，img是二值化的边缘图像，minr是最小搜索的圆半径，
maxr是最大搜索的圆半径，stepr是圆半径增长步长，stepa是角度增长步长，percent是阈值百分比，返回circlefind，存放了大于阈值的圆的圆心坐标以及半径。
